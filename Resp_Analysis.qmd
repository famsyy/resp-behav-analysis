...
-   The following analysis pipeline was coded to analyze reaction time and accuracy score data from an 2-back n-back task performed by younger (18-35 years older) and older (60+) healthy adults after completing different breathing protocols.
...
root <- setwd("C:/Users//DATA") 




# Pick Corresponding Wide Data Frame
summaryDF_wide <- read_csv("C:/Users//summaryDATA_wide.csv") 
...
final_df <- final_df %>%
  mutate(group=
           ifelse(substr(participant, 1, 1) =="A", "older",
...
# Includes both younger and older Adults
np <- final_df_onlyCor %>%
  distinct(participant) %>%
  n_distinct()

# Includes only older Adults
np_older <- final_df_onlyCor %>%
  filter(group == "older")%>%
  distinct(participant) %>%
  n_distinct()
...
plot_spont_sf <- plot_sf(out_spont_sf, plot_theme = 1)[[1]] + 
     theme(axis.text = element_text(size = 16, colour="black"))+labs(title = "SF - younger & older Adults - Spont") # Plot
plot_spont_sf

# Hierarchical Shift Function
out_spont_hsf <- hsf(hsf_spont_tibble, rt ~ cond + id)

plot_spont_hsf <- plot_hsf(out_spont_hsf)+labs(title = "HSF - younger & older Adults - Spont")
plot_spont_hsf
...
out_spont_hsf_ci <- plot_hsf_pb(hsf_pb_spont, interv = "ci") +labs(title = "HSF - younger & older Adults - Spont + CI")
out_spont_hsf_ci

# Plot Boostrap Distributions
set.seed(8899)
out_spont_hsf_boot <- plot_hsf_pb_dist(hsf_pb_spont)+labs(title = "Boostrap Difference Distributions - younger & older Adults - Spont")
out_spont_hsf_boot
...
# Filter DF to older Adults
hsf_spont_older <- hsf_spont %>% 
  dplyr::filter(group=="older")

# Create Tibble
hsf_spont_tibble_older <- tibble(rt = hsf_spont_older$key_resp.rt,
             cond = factor(hsf_spont_older$thisCond),
             id = factor(hsf_spont_older$participant))

# Shift Function
out_spont_sf_older <- shifthd_pbci(hsf_spont_tibble_older, rt ~ cond, doall = TRUE, nboot = 200, adj_ci = TRUE) # Function

plot_spont_sf_older <- plot_sf(out_spont_sf_older, plot_theme = 1)[[1]] + 
     theme(axis.text = element_text(size = 16, colour="black"))+labs(title = "SF - older Adults - Spont") # Plot
plot_spont_sf_older

# Hierarchical Shift Function
out_spont_hsf_older <- hsf(hsf_spont_tibble_older, rt ~ cond + id)

plot_spont_hsf_older <- plot_hsf(out_spont_hsf_older)+labs(title = "HSF - older Adults - Spont")
plot_spont_hsf_older


# Percentile Boostrap Hierarchical Shift Function
set.seed(8899)
hsf_pb_spont_older <- hsf_pb(hsf_spont_tibble_older, rt ~ cond + id) # Boostrap Samples

# Plot Confidence Intervals
out_spont_hsf_ci_older <- plot_hsf_pb(hsf_pb_spont_older, interv = "ci") +labs(title = "HSF - older Adults - Spont + CI")
out_spont_hsf_ci_older

# Plot Boostrap Distributions
set.seed(8899)
out_spont_hsf_boot_older <- plot_hsf_pb_dist(hsf_pb_spont_older)+labs(title = "Boostrap Difference Distributions - older Adults - Spont")
out_spont_hsf_boot_older


# P Values
out_spont_hsf_older$pvalues


# Adjusted P Values
out_spont_hsf_older$adjusted_pvalues

# Stochastic Dominance
# Participants with all quantile differences > 0
nq_spont_older <- length(out_spont_hsf_older$quantiles)
pdmt0_spont_older <- apply(out_spont_hsf_older$individual_sf > 0, 2, sum)
print(paste0('In ',sum(pdmt0_spont_older == nq_spont_older),' participants (',round(100 * sum(pdmt0_spont_older == nq_spont_older) / np_older, digits = 1),'%), all quantile differences are more than to zero'))

# Participants with all quantile differences < 0
pdlt0_spont_older <- apply(out_spont_hsf_older$individual_sf < 0, 2, sum)
print(paste0('In ',sum(pdlt0_spont_older == nq_spont_older),' participants (',round(100 * sum(pdlt0_spont_older == nq_spont_older) / np_older, digits = 1),'%), all quantile differences are less than to zero'))
...
plot_slow_sf <- plot_sf(out_slow_sf, plot_theme = 1)[[1]] + 
     theme(axis.text = element_text(size = 16, colour="black"))+labs(title = "SF - younger & older Adults - Slow") # Plot
plot_slow_sf

# Hierarchical Shift Function
out_slow_hsf <- hsf(hsf_slow_tibble, rt ~ cond + id)

plot_slow_hsf <- plot_hsf(out_slow_hsf)+labs(title = "HSF - younger & older Adults - Slow")
plot_slow_hsf
...
# Plot Confidence Intervals
out_slow_hsf_ci <- plot_hsf_pb(hsf_pb_slow, interv = "ci") +labs(title = "HSF - younger & older Adults - Slow + CI")
out_slow_hsf_ci

# Plot Boostrap Distributions
set.seed(8899)
out_slow_hsf_boot <- plot_hsf_pb_dist(hsf_pb_slow)+labs(title = "Boostrap Difference Distributions - younger & older Adults - Slow")
out_slow_hsf_boot
...
# Filter DF to older Adults
hsf_slow_older <- hsf_slow %>% 
  dplyr::filter(group=="older")

# Create Tibble
hsf_slow_tibble_older <- tibble(rt = hsf_slow_older$key_resp.rt,
             cond = factor(hsf_slow_older$thisCond),
             id = factor(hsf_slow_older$participant))

# Shift Function
out_slow_sf_older <- shifthd_pbci(hsf_slow_tibble_older, rt ~ cond, doall = TRUE, nboot = 200, adj_ci = TRUE) # Function

plot_slow_sf_older <- plot_sf(out_slow_sf_older, plot_theme = 1)[[1]] + 
     theme(axis.text = element_text(size = 16, colour="black"))+labs(title = "SF - older Adults - Slow") # Plot
plot_slow_sf_older

# Hierarchical Shift Function
out_slow_hsf_older <- hsf(hsf_slow_tibble_older, rt ~ cond + id)

plot_slow_hsf_older <- plot_hsf(out_slow_hsf_older)+labs(title = "HSF - older Adults - Slow")
plot_slow_hsf_older


# Percentile Boostrap Hierarchical Shift Function
set.seed(8899)
hsf_pb_slow_older <- hsf_pb(hsf_slow_tibble_older, rt ~ cond + id) # Boostrap Samples

# Plot Confidence Intervals
out_slow_hsf_ci_older <- plot_hsf_pb(hsf_pb_slow_older, interv = "ci") +labs(title = "HSF - older Adults - Slow + CI")
out_slow_hsf_ci_older

# Plot Boostrap Distributions
set.seed(8899)
out_slow_hsf_boot_older <- plot_hsf_pb_dist(hsf_pb_slow_older)+labs(title = "Boostrap Difference Distributions - older Adults - Slow")
out_slow_hsf_boot_older


# P Values
out_slow_hsf_older$pvalues


# Adjusted P Values
out_slow_hsf_older$adjusted_pvalues

# Stochastic Dominance
# Participants with all quantile differences > 0
nq_slow_older <- length(out_slow_hsf_older$quantiles)
pdmt0_slow_older <- apply(out_slow_hsf_older$individual_sf > 0, 2, sum)
print(paste0('In ',sum(pdmt0_slow_older == nq_slow_older),' participants (',round(100 * sum(pdmt0_slow_older == nq_slow_older) / np_older, digits = 1),'%), all quantile differences are more than to zero'))

# Participants with all quantile differences < 0
pdlt0_slow_older <- apply(out_slow_hsf_older$individual_sf < 0, 2, sum)
print(paste0('In ',sum(pdlt0_slow_older == nq_slow_older),' participants (',round(100 * sum(pdlt0_slow_older == nq_slow_older) / np_older, digits = 1),'%), all quantile differences are less than to zero'))
...
# Dropping fast condition due to no older Adult fast condition data
...
ReactionTimeANCOVA <- cat_plot(model = fit, pred = thisCond, modx = group, plot.points = TRUE)+ scale_x_discrete(limits=c("pre_spont", "post_spont", "pre_slow", "post_slow"))+labs(x="Breathing Condition", y="Reaction Time - Seconds", title = "ANCOVA - younger & older Adults - Reaction Time")
...
ANOVA_Plot_older <- ggbetweenstats(
  ...
  x = thisCond,
  y = key_resp.rt
)+ scale_x_discrete(limits=c("pre_spont", "post_spont", "pre_slow", "post_slow"))+labs(x="Breathing Condition", y="Reaction Time - Seconds", title = "ANOVA - older Adults Reaction Time", subtitle = "", caption="")
...
scale_x_continuous(breaks=c(1,2), labels=c("Pre Slow", "Post Slow"), limits=c(0, 3)) +
  ...
  labs(x = "Breathing Condition", y = "Reaction Time - Seconds",
       title = "(A) - T-Test forBoth younger & older Adults - Reaction Time - Slow Breathing")
...
scale_x_continuous(breaks=c(1,2), labels=c("Pre Slow", "Post Slow"), limits=c(0, 3)) +
  theme_classic()+
  labs(x = "Breathing Condition", y = "Reaction Time - Seconds",
       title = "(B) - T-Test for older Adults - Reaction Time - Slow Breathing") 
...
# young Slow
Nonpara_t_prepost_slow_young <- wilcox.test(data_prepost_young_Slow$pre_slow_avg_rt, data_prepost_young_Slow$post_slow_avg_rt, paired = T)
report(Nonpara_t_prepost_slow_young)

# young Fast
Nonpara_t_prepost_fast_young <- wilcox.test(data_prepost_young_Fast$pre_fast_avg_rt, data_prepost_young_Fast$post_fast_avg_rt, paired = T)
...
# Drop fast conditions due to lack of older Adult group fast data
...
AccuracyPlot <- cat_plot(model = fit2, pred = thisCond, modx = group, plot.points = TRUE)+ scale_x_discrete(limits=c("pre_spont", "post_spont", "pre_slow", "post_slow"))+labs(x="Breathing Condition", y="Accuracy Score - %", title = "ANCOVA - younger & older Adults - Accuracy")
...
# ANOVA for older Adults - Accuracy
...
ANOVA_Plot_Corr_older <- ggbetweenstats(
  ...
  x = thisCond,
  y = key_resp.corr
)+ scale_x_discrete(limits=c("pre_spont", "post_spont", "pre_slow", "post_slow"))+labs(x="Breathing Condition", y="Accuracy - %", title = "ANOVA - older Adults - Accuracy ", subtitle = "", caption="")
...
scale_x_continuous(breaks=c(1,2), labels=c("Pre Slow", "Post Slow"), limits=c(0, 3)) +
  ...
  labs(x = "Breathing Condition", y = "Accuracy - %",
       title = "(A) - T-Test young & older Adults - Accuracy - Slow Breathing") 
...
scale_x_continuous(breaks=c(1,2), labels=c("Pre Slow", "Post Slow"), limits=c(0, 3)) +
  theme_classic()+
  labs(x = "Breathing Condition", y = "Accuracy - %",
       title = "(B) - older Adult Slow Breathing") 
...
# older Adults - Accuracy - Slow Breathing
DACF_older_Adults_df <- summaryDF_wide %>% 
  dplyr::filter(group=="older")

DACF_t_Corr_slow_older <- lw.t.test(DACF_older_Adults_df$pre_slow_avg_corr, DACF_older_Adults_df$post_slow_avg_corr, "a")
...
Groups_Corr_Power_Plot <- ggplot(p_valuesGROUP_Corr, aes(x = sample_size, y = avg_p_value)) +
  ...
  labs(x = "Sample Size", y = "Average P-Value",
       title = "(A) - older & younger Adult Slow Breathing")
...
## (Acc) Power Analysis - older ADULTS PRE/POST SLOW
...
## (RT) Power Analysis - older ADULTS PRE/POST SLOW
